...
Once you are done creating the form, make a `POST` request to the API endpoint `https://f5-beers-065cad3017be.herokuapp.com/beers`, passing all the input values in the request `body` as an object. The fields of the request `body` should have exact **names** so that the API can create a new beer.
